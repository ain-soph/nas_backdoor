...
# ASR, ACC and scores of arches perturbed from '|{0}~0|+|nor_conv_1x1~0|nor_conv_1x1~1|+|{1}~0|nor_conv_1x1~1|nor_conv_3x3~2|'

r"""
CUDA_VISIBLE_DEVICES=0 python ./trajectory.py --verbose 1 --model nats_bench --attack input_aware_dynamic --validate_interval 1 --train_mask_epochs 10 --epochs 10 --lr 1e-2 --natural
"""  # noqa: E501
...
if __name__ == '__main__':
    ...
    arch_str_pattern = '|{0}~0|+|nor_conv_1x1~0|nor_conv_1x1~1|+|{1}~0|nor_conv_1x1~1|nor_conv_3x3~2|'

    for first in atoms:
        for second in atoms:
            arch_str = arch_str_pattern.format(first, second)
            ...
            logger.create_meters(clean_score='{median:.3f}', model_score='{median:.3f}', generator_score='{median:.3f}')
            ...
            for model_seed in [777, 888, 999]:
                ...
                for data in logger.log_every(dataset.loader['train'], header=header):
                    ...
                    clean_model_params = {name: param for name, param in model.named_parameters() if 'weight' in name}
